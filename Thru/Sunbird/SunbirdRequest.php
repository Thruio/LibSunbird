...
class SunbirdRequest{
  ...
  private function get_type(){
    return strtolower(str_replace("Fapla\\Models\\Sunbird","", get_called_class()));
  }
  ...
  public function get_list() {
    ...
    $output = [];
    $child_class = "Fapla\\Models\\Sunbird" . Inflect::singularize($list);
  ...
  private function request($mode = 'get'){
    ...
    if($this->curl->get_status() != 200){
      throw new SunbirdException("Cannot talk to Sunbird, response was {$this->curl->get_status()}");
    }
